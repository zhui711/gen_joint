#!/usr/bin/env python3
"""
Visualize Mask-Weighted Feature Maps for Anatomy-Aware Loss Analysis.

This script visualizes what the segmentation model's encoder "sees" when
processing GT and generated CXR images. It supports both:
  1. Raw feature heatmaps (global view)
  2. Mask-weighted organ-specific feature heatmaps (per-organ isolation)

The mask-weighted mode replicates the exact computation used in loss_anatomy_v2.py:
  OrganFeature^c = M^c * F  (at 1/4 resolution)
...
Layer Selection (ResNet34 encoder, 256x256 input):
    ...
    Feature 3: (B, 128, 32, 32)  stride=8  - 1/8 resolution
    Feature 4: (B, 256, 16, 16)  stride=16 - 1/16 resolution
"""
...
# Anatomical channel names (must match loss_anatomy_v2.py)
ANATOMY_CHANNELS = [
    "Lung_Left",      # 0
    "Lung_Right",     # 1
    "Heart",          # 2
...
def load_and_preprocess_image(image_path, target_size=256):
    ...
    Returns:
        ...
        original_resized: numpy (H, W, 3) in [0, 255] for overlay
    """
...
def extract_all(model, image_tensor, device):
    ...
    Returns:
        features: list of encoder feature tensors
        mask_pred: (1, 10, 256, 256) sigmoid probabilities
    """
    image_tensor = image_tensor.to(device)
    with torch.no_grad():
        features = model.encoder(image_tensor)
        # Reuse encoder features for mask prediction (no redundant forward pass)
...
# =============================================================================
# Visualization: Raw Feature Heatmaps (existing functionality)
# =============================================================================
...
# =============================================================================
# Visualization: Mask-Weighted Organ-Specific Feature Heatmaps (NEW)
# =============================================================================
...
def create_organ_feature_figure(
    ...
    """
    Per-organ mask-weighted feature visualization.

    For each organ channel c:
      OrganFeature_gt^c = M_gt^c * F_gt  (channel-averaged -> 2D heatmap)
      OrganFeature_gen^c = M_gen^c * F_gen

    Layout: rows = organs, cols = [GT Mask, GT OrganFeat, Gen Mask, Gen OrganFeat, Diff]
    """
    ...
    for row, c in enumerate(organ_channels):
        ...
        # Extract single-channel mask: (1, 1, H_feat, W_feat)
        M_gt_c = mask_gt_down[:, c:c+1, :, :]
        M_gen_c = mask_gen_down[:, c:c+1, :, :]

        # Compute isolated organ features
        OrganFeat_gt = M_gt_c * F_gt    # (1, C, H, W)
        OrganFeat_gen = M_gen_c * F_gen  # (1, C, H, W)

        # Channel-average to 2D spatial heatmap
        ...
        mask_gt_orig_c = mask_gt[0, c].cpu().numpy()   # (256, 256)
        mask_gen_orig_c = mask_gen[0, c].cpu().numpy()  # (256, 256)

        # Per-organ MSE (same as loss computation)
        mse_c = F.mse_loss(OrganFeat_gen, OrganFeat_gt).item()
        ...
        # Col 1: GT organ feature heatmap overlaid on image
        ...
        # Col 2: Gen original with mask overlay
        ...
        axes[row, 2].set_title(f"Gen Mask: {organ_name}", fontsize=9)
        axes[row, 2].axis("off")

        # Col 3: Gen organ feature heatmap overlaid on image
        ...
        axes[row, 3].set_title(f"Gen OrganFeat", fontsize=9)
        axes[row, 3].axis("off")

        # Col 4: Difference heatmap
        diff_2d = np.abs(organ_gt_2d - organ_gen_2d)
        diff_heatmap = feature_to_heatmap(diff_2d, cv2.COLORMAP_HOT)
        diff_heatmap_resized = cv2.resize(diff_heatmap, (256, 256))
        axes[row, 4].imshow(diff_heatmap_resized)
        axes[row, 4].set_title(f"|Diff| MSE={mse_c:.4f}", fontsize=9)
        axes[row, 4].axis("off")
        ...
        axes[row, 5].set_title(f"Raw Diff\nMSE={raw_mse:.4f}", fontsize=9)
        axes[row, 5].axis("off")

    layer_name = f"Layer{feature_layer_idx} (1/{2**feature_layer_idx} res)"
    plt.suptitle(
        f"Mask-Weighted Organ Feature Analysis — {layer_name}",
...
def create_loss_summary_table(
    ...
    """Print per-organ MSE table (replicates loss_anatomy_v2 computation)."""
    F_gt = gt_features[feature_layer_idx]
    F_gen = gen_features[feature_layer_idx]
    _, _, H_feat, W_feat = F_gt.shape
    ...
    print("\n" + "=" * 65)
    print(f"Per-Organ MSE (Layer {feature_layer_idx}, {H_feat}x{W_feat} resolution)")
    print("=" * 65)
    ...
    for c in range(10):
        ...
        print(f"  Ch {c:2d} [{ANATOMY_CHANNELS[c]:>15s}]  MSE={mse_c:.6f}  "
              f"coverage={mask_coverage:.1f}%")

    print("-" * 65)
    print(f"  Total Loss (sum): {total_loss:.6f}")
    print("=" * 65)


def main():
    parser = argparse.ArgumentParser(
        description="Visualize mask-weighted encoder features for anatomy-aware loss."
    )
    ...
    parser.add_argument("--mask_gt", type=str, default=None,
                        help="Path to GT mask .npz file (key='mask', shape 10x256x256). "
                             "If provided, enables mask-weighted organ visualization.")
    ...
    # Extract features + predicted masks
    ...
    print(f"\nEncoder feature structure:")
    for i, f in enumerate(gt_features):
        stride = args.target_size // f.shape[-1]
        print(f"  Feature {i}: shape={tuple(f.shape)}, stride={stride}")

    # === 1. Raw feature summary (always generated) ===
    ...
    # === 2. Mask-weighted organ visualization (if mask provided) ===
    if args.mask_gt is not None:
        print(f"\nLoading GT mask: {args.mask_gt}")
        mask_data = np.load(args.mask_gt)
        mask_gt_np = mask_data["mask"].astype(np.float32)  # (10, 256, 256)
        mask_gt_tensor = torch.from_numpy(mask_gt_np).unsqueeze(0).to(args.device)  # (1, 10, 256, 256)

        # Use predicted mask for gen image (same as training loss)
        mask_gen_tensor = mask_pred_gen  # (1, 10, 256, 256)

        # Validate organ channels
        valid_channels = [c for c in args.organ_channels if 0 <= c < 10]
        if not valid_channels:
            print("WARNING: No valid organ channels specified, using all 10.")
            valid_channels = list(range(10))

        print(f"Visualizing organs: {[ANATOMY_CHANNELS[c] for c in valid_channels]}")
        ...
        create_organ_feature_figure(
            ...
            output_path=os.path.join(args.output_dir, "organ_feature_analysis.png"),
        )

        # Print per-organ MSE table
        ...
        print("To enable, pass: --mask_gt /path/to/mask.npz")
