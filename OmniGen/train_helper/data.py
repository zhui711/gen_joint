...
class DatasetFromJson(torch.utils.data.Dataset):
    ...
    @staticmethod
    def load_output_mask(mask_path, key="mask"):
        """Load a boolean .npz mask and return as float32 tensor (C, H, W)."""
        data = np.load(mask_path)
        mask = data[key].astype(np.float32)  # (10, 256, 256)
    ...
    def get_example(self, index):
        ...
        # Load anatomy mask if present in JSONL
        output_mask = example.get('output_mask', None)
        if output_mask is not None:
            output_anatomy_mask = self.load_output_mask(output_mask)
...
class TrainDataCollator(OmniGenCollator):
    def __init__(self, pad_token_id: int, hidden_size: int, keep_raw_resolution: bool, num_mask_tokens: int = 0):
        self.pad_token_id = pad_token_id
        self.hidden_size = hidden_size
        self.keep_raw_resolution = keep_raw_resolution
        self.num_mask_tokens = num_mask_tokens  # Extra tokens for joint mask co-generation
    ...
    def __call__(self, features):
        ...
        # Stack anatomy masks if present (backward compatible)
        has_masks = len(features) > 0 and len(features[0]) > 2
        if has_masks:
            data["output_anatomy_masks"] = torch.stack([f[2] for f in features], dim=0)
