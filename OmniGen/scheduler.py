...
class OmniGenScheduler:
    ...
    def __call_joint__(self, z_img, z_mask, func, model_kwargs, use_kv_cache: bool=True, offload_kv_cache: bool=True):
        """Joint image-mask co-denoising scheduler.

        Both z_img and z_mask are updated with the same timestep schedule.
        The func must return ((img_pred, mask_pred), cache).

        Args:
            z_img: (B, 4, H, W) image noise latent
            z_mask: (B, 4, H, W) mask noise latent
            func: Model forward function (forward_with_cfg or forward_with_separate_cfg)
            model_kwargs: Dict of model keyword arguments
            use_kv_cache: Whether to use KV cache
            offload_kv_cache: Whether to offload KV cache to CPU

        Returns:
            z_img: Denoised image latent
            z_mask: Denoised mask latent
        """
        ...
        if isinstance(model_kwargs['input_ids'], list):
            cache = [OmniGenCache(num_total_gen_tokens, offload_kv_cache) for _ in range(len(model_kwargs['input_ids']))] if use_kv_cache else None
        else:
            cache = OmniGenCache(num_total_gen_tokens, offload_kv_cache) if use_kv_cache else None
        ...
        for i in tqdm(range(self.num_steps)):
            ...
            if i == 0 and use_kv_cache:
                ...
                # Crop for total generated tokens (img + mask)
                model_kwargs['position_ids'] = self.crop_position_ids_for_cache(model_kwargs['position_ids'], num_total_gen_tokens)
                model_kwargs['attention_mask'] = self.crop_attention_mask_for_cache(model_kwargs['attention_mask'], num_total_gen_tokens)
