...
class OmniGenProcessor:
    ...
    def __call__(self,
        ...
        if separate_cfg_input:
            return self.separate_collator(input_data, num_mask_tokens=num_mask_tokens)
        return self.collator(input_data, num_mask_tokens=num_mask_tokens)
...
class OmniGenCollator:
    ...
    def __call__(self, features, num_mask_tokens: int = 0):
        ...
        if num_mask_tokens > 0:
            all_padded_input_ids, all_position_ids, all_attention_mask, all_padding_images, all_pixel_values, all_image_sizes = self.process_mllm_input_joint(mllm_inputs, target_img_size, num_mask_tokens)
        else:
...
class OmniGenSeparateCollator(OmniGenCollator):
    def __call__(self, features, num_mask_tokens: int = 0):
        ...
        _process = self.process_mllm_input_joint if num_mask_tokens > 0 else self.process_mllm_input
        _extra = (num_mask_tokens,) if num_mask_tokens > 0 else ()

        padded_input_ids, position_ids, attention_mask, padding_images, pixel_values, image_sizes = _process(mllm_inputs, target_img_size, *_extra)
        ...
        if cfg_mllm_inputs[0] is not None:
            padded_input_ids, position_ids, attention_mask, padding_images, pixel_values, image_sizes = _process(cfg_mllm_inputs, target_img_size, *_extra)
            all_padded_input_ids.append(padded_input_ids)
            all_attention_mask.append(attention_mask)
            all_position_ids.append(position_ids)
            all_pixel_values.append(pixel_values)
            ...
            all_padding_images.append(padding_images)
        if img_cfg_mllm_input[0] is not None:
            padded_input_ids, position_ids, attention_mask, padding_images, pixel_values, image_sizes = _process(img_cfg_mllm_input, target_img_size, *_extra)
            all_padded_input_ids.append(padded_input_ids)
            all_attention_mask.append(attention_mask)
            all_position_ids.append(position_ids)
            all_pixel_values.append(pixel_values)
